...
# Create a function that just simply generate the data.
...
# Creating a functio to train the model.
def train_model():
...
def main():
    st.title("Get Your Dream House Under.....")
    
    st.write("Put in your house size to know its price.")

    model = train_model()

    size = st.number_input('House size', min_value = 300, max_value = 4000, value= 1500)

    if st.button('Predict Price'):
        predicted_price = model.predict([[size]])
        st.success(f'Estimated Price: ${float(predicted_price[0]):,.2f}')
